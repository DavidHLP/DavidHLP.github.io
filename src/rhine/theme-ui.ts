...
export function themeSettingsMarkup(dark: boolean) {
  return `<div class="theme-settings"><div><strong>${rt("theme.label")}</strong><span>${rt("theme.description")}</span></div><div class="theme-choices" role="group" aria-label="${rt("theme.aria")}"><button data-color-theme="light" aria-pressed="${!dark}">${rt("theme.light")}</button><button data-color-theme="dark" aria-pressed="${dark}">${rt("theme.dark")}</button></div></div>`;
}
