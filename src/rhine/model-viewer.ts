...
const PARTS = [
  { id: "fasteners", en: "FASTENERS", depth: 2.75 },
  { id: "cover", en: "OPTICAL COVER", depth: 1.85 },
  {
    id: "optical-lenses",
    en: "REFRACTIVE RINGS",
    depth: 0.75,
  },
  { id: "optical-core", en: "OPTICAL CORE", depth: -0.15 },
...
export class ModelViewer {
  ...
  constructor(
    ...
    this.root.innerHTML = `
      ...
      <header class="viewer-header">
        <button class="viewer-back" data-viewer="close">← <span>${rt("viewer.back")}</span><kbd>ESC</kbd></button>
        <div class="viewer-heading"><span>RHINE LAB / ${rt("viewer.objectStudy")}</span><h2 id="viewer-title">${rt("viewer.title")}</h2><p id="viewer-file"></p></div>
      ...
      <footer class="viewer-footer">
        ...
        <div class="viewer-actions"><button data-viewer="explode" aria-pressed="false"><span>＋</span> ${rt("viewer.explode")}</button><button data-viewer="assemble" aria-pressed="true"><span>−</span> ${rt("viewer.assemble")}</button></div>
        <button class="viewer-reset" data-viewer="reset">${rt("viewer.reset")} <span>↗</span></button>
      </footer>
      <div class="viewer-state" aria-live="polite">${rt("viewer.assembled")}</div>`;
  ...
  open(
    ...
    this.root.querySelector("#viewer-file")!.textContent =
      `${rt("archive.fileLabel")} ${id} / ${rt("archive.database")}`;
  ...
  private async load() {
    ...
    loading.querySelector("span")!.textContent = rt("viewer.loading");
    ...
    try {
      ...
      this.setExploded(false);
      this.setStatus(rt("viewer.assembled"));
      ...
      this.loading = false;
      loading.querySelector("span")!.textContent = rt("viewer.failed");
  ...
  private setExploded(value: boolean) {
    ...
    this.root
      ...
      .setAttribute("aria-pressed", String(!value));
    this.setStatus(
      value ? rt("viewer.disassembling") : this.spread.value > 0.001 ? rt("viewer.reassembling") : rt("viewer.assembled"),
    );
  ...
  resize() {
    ...
    help.innerHTML = touch
      ? `<span>${rt("viewer.helpTouchRotate")}</span><span>${rt("viewer.helpTouchZoom")}</span>`
      : `<span>${rt("viewer.helpRotate")}</span><span>${rt("viewer.helpPan")}</span><span>${rt("viewer.helpZoom")}</span>`;
  }
  ...
  update(time: number) {
    ...
    if (this.source) {
      ...
      if (
        ...
        this.setStatus(this.targetSpread ? rt("viewer.disassembled") : rt("viewer.assembled"));
      }
      for (const part of PARTS) {
