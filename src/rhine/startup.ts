...
export class StartupGate {
  ...
  constructor(private options: EntryOptions) {
    ...
    root.setAttribute("aria-label", rt("startup.aria"));
    root.insertAdjacentHTML("beforeend", `<div class="entry-controls"><button class="entry-start" disabled>${rt("startup.preparing")}</button><button class="entry-silent" hidden>${rt("startup.silent")}</button><p class="entry-status" role="status">${rt("startup.ready")}</p></div>`);
  ...
  ready() {
    ...
    this.button.textContent = rt("startup.enter");
    this.options.root.querySelector(":scope > span")!.textContent = rt("startup.databaseReady");
    this.status.textContent = rt("startup.touch");
  ...
  private async enter() {
    ...
    this.button.textContent = rt("startup.preparingSound");
    this.status.textContent = rt("startup.readyStart");
    ...
    try {
      ...
      if (unlocked && !document.hidden) this.finish(false);
        ...
        this.button.textContent = rt("startup.retry");
        this.status.textContent = rt("startup.audioNotReady");
      }
    } catch {
      if (request !== this.request) return;
      this.options.cancel();
