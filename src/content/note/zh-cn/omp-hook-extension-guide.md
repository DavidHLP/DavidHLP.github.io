...
- **上下文与 Agent 回合**：`context`、`before_agent_start`、`agent_start`、`agent_end`、`turn_start`、`turn_end`。
- **自动恢复与提醒**：`auto_compaction_start`、`auto_compaction_end`、`auto_retry_start`、`auto_retry_end`、`ttsr_triggered`、`todo_reminder`。
- **工具边界**：`tool_call`、`tool_result`；其中 `tool_call` 携带 `toolName`、`toolCallId`、`input`，handler 可返回阻断结果。
...
- Hook 由 loader 通过 capability API 发现并结合显式配置路径加载；`hooks/pre`、`hooks/post` 子目录自动扫描在 v17.2.15 的 loader 证据中未出现，不应写成默认约定。
- `session_start` 可建立探针、缓存项目根或初始化状态；`tool_call` 应只做快速、可失败的分类，避免阻塞工具路径。
- `setStatus` 是“事件 → 状态 map → 渲染器”的桥。v17.2.15 的 `statusLine` segment ID 是封闭 union，共 24 项且没有 `hook`；配置不会凭空增加新的 Hook segment。公开 PR #2001 closed 且未合并，不能当作主线能力。
- 状态可见性和调用强制性必须解耦：状态行用于观察，阻断结果用于一次调用的控制，真正的安全边界仍应由独立的权限/写门禁承担。

### 3. HookCommandContext 的死锁安全边界

扩展命令上下文提供 `newSession`、`branch`、`navigateTree`，但有意省略 `switchSession`、`reload`、`compact`、`getContextUsage`。这些动作若从 `tool_call`、`context` 等事件处理器内调用可能造成死锁，因此“缺省不可用”是安全边界，不是待补齐的 API。需要执行被省略的动作时，应把请求移到事件处理器之外，并在新会话中验证。
...
- **来源事实**：`legacy-omp-hook-extension-guide` 记录早期 `session_start`/`tool_call`、`HookAPI.sendMessage`、`ToolCallEventResult`、`ctx.ui.setStatus`、mock-pi 与 tmux 分层验证；`omp-17-2-15-runtime-contract` 保存最初的 v17.2.15 固定 tag 快照，`omp-17-2-15-runtime-contract-correction` 另行补充 ExtensionRunner、完整事件分组、`sendMessage`/`appendEntry` 分流、HookCommandContext 的死锁安全省略项，以及无 `hook` 的封闭 statusline union。
- **本页综合**：把“事件全集”“LLM 消息/状态记录”“决策干预”和“可见性桥接”分成因果链，并把软提示明确降级为建议机制，是为了避免把可观察性误报成强制安全。
- **版本与未确认项**：上述官方事实只适用于 OMP `17.2.15` 固定 tag；事件是否触发、默认 `display`/`triggerTurn`、状态行开关、具体加载路径和热加载行为仍需按安装版本与新会话复核。`hooks/pre`/`post` 自动扫描不是本页确认的事实。
