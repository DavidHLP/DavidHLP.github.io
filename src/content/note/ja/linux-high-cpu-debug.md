...
`htop` を入力すると、よりriendlyなインターフェースになり、ヘッダーのクリックソートやより明確な色分けがサポートされます。
...
完全に一時停止したくない，只是想让CPU使用率降低，可以将优先级调到最低（Nice值最大为19）：
...
高CPU的原因是大量のファイルI/O待受やネットワークパケット処理の場合があります。
