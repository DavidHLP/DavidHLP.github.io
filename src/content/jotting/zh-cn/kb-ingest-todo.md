...
- [x] 哪些页面应该合并为共享概念，哪些应该保留为不同实体的局部实现？——2026-08-21 审计结论：Redis 三页（业务模式/持久化原理/黑马实战）来源与视角不同，保留分页并以 `related` 互链；Headroom/OMP 七页按契约、演进、共存、持久化分工明确，暂不合并；series 分类已归并为 8 个正典分区（见 kb-log 2026-08-21 maintenance）。
