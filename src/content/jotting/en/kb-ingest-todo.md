...
- [x] 个人项目 `ResiCache`（固定提交 `75ed279a`）与 `UltiCode`（固定提交 `3f14ac89`）的 README/CONTEXT 已完成 raw → entity 页闭环（正典见 zh-cn `/note/resicache` 与 `/note/ulticode`）；运行证据（压测、沙箱隔离强度、生产部署）仍待补充。
