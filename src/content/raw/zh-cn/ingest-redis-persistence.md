---
title: Redis 持久化与原理聚合快照：RDB/AOF、数据结构与高级机制
...
description: 聚合 Redis 持久化与原理 3 篇原文（personal 2 篇 + fuwari 1 篇），固定 personal bbb2126 / fuwari 07cee2b，含黑马原理篇 56KB 深度内容。
---

# Redis 持久化与原理聚合快照：RDB/AOF、数据结构与高级机制

本文件为聚合证据快照（immutable raw），按 LLM-Wiki 规范原样收录多篇来源原文，不改动正文，仅增加 provenance 头部与分隔。后续 wiki 页通过 `sources: ["{slug}"]` 引用本快照。

- raw slug: `ingest-redis-persistence`
- 对应 wiki: `redis-persistence-principle`
...
| 序号 | 仓库 | 相对路径 | 大小 | 去重标注 |
| --- | --- | --- | --- | --- |
| 1 | Personal-markdown-notes | `redis/高级/Redis持久化.md` | 9666 |  |
| 2 | Personal-markdown-notes | `redis/黑马/原理/原理篇.md` | 56351 |  |
| 3 | Fuwari | `redis/principle/RedisPersistence.md` | 9444 |  |
...
## 来源 1: Personal-markdown-notes / `redis/高级/Redis持久化.md`

- 原始 URL: <https://github.com/DavidHLP/Personal-markdown-notes/blob/bbb21260029584d41d1c667f88c5c8e2b761aad9/redis/高级/Redis持久化.md>
- 本地路径: `redis/高级/Redis持久化.md`
...
[image: RDB持久化原理](images/Redis持久化/image-20210725151319695.png)
...
[image: AOF持久化流程](images/Redis持久化/image-20210725151543640.png)
...
[image: AOF同步策略对比](images/Redis持久化/image-20210725151654046.png)
...
[image: AOF重写过程](images/Redis持久化/image-20210725151729118.png)
...
[image: RDB与AOF对比](images/Redis持久化/image-20210725151940515.png)
...
## 来源 2: Personal-markdown-notes / `redis/黑马/原理/原理篇.md`

- 原始 URL: <https://github.com/DavidHLP/Personal-markdown-notes/blob/bbb21260029584d41d1c667f88c5c8e2b761aad9/redis/黑马/原理/原理篇.md>
- 本地路径: `redis/黑马/原理/原理篇.md`
...
[image: 1653984583289](.\原理篇.assets\1653984583289.png)

那么Redis将在底层创建两个SDS，其中一个是包含“name”的SDS，另一个是包含“虎哥”的SDS。

Redis是C语言实现的，其中SDS是一个结构体，源码如下：

[image: 1653984624671](.\原理篇.assets\1653984624671.png)

例如，一个包含字符串“name”的sds结构如下：

[image: 1653984648404](.\原理篇.assets\1653984648404.png)

SDS之所以叫做动态字符串，是因为它具备动态扩容的能力，例如一个内容为“hi”的SDS：

[image: 1653984787383](.\原理篇.assets\1653984787383.png)
...
[image: 1653984822363](.\原理篇.assets\1653984822363.png)

[image: 1653984838306](.\原理篇.assets\1653984838306.png)
...
[image: 1653984923322](.\原理篇.assets\1653984923322.png)

其中的encoding包含三种模式，表示存储的整数大小不同：

[image: 1653984942385](.\原理篇.assets\1653984942385.png)

为了方便查找，Redis会将intset中所有的整数按照升序依次保存在contents数组中，结构如图：

[image: 1653985149557](.\原理篇.assets\1653985149557.png)
...
[image: 1653985197214](.\原理篇.assets\1653985197214.png)
...
[image: 1653985276621](.\原理篇.assets\1653985276621.png)

源码如下：

[image: 1653985304075](.\原理篇.assets\1653985304075.png)

[image: 1653985327653](.\原理篇.assets\1653985327653.png)
...
[image: 1653985396560](.\原理篇.assets\1653985396560.png)

当我们向Dict添加键值对时，Redis首先根据key计算出hash值（h），然后利用 h & sizemask来计算元素应该存储到数组中的哪个索引位置。我们存储k1=v1，假设k1的哈希值h =1，则1&3 =1，因此k1=v1要存储到数组角标1位置。

[image: 1653985497735](.\原理篇.assets\1653985497735.png)

Dict由三部分组成，分别是：哈希表（DictHashTable）、哈希节点（DictEntry）、字典（Dict）

[image: 1653985570612](.\原理篇.assets\1653985570612.png)

[image: 1653985586543](.\原理篇.assets\1653985586543.png)

[image: 1653985640422](.\原理篇.assets\1653985640422.png)
...
[image: 1653985716275](.\原理篇.assets\1653985716275.png)

[image: 1653985743412](.\原理篇.assets\1653985743412.png)
...
[image: 1653985824540](.\原理篇.assets\1653985824540.png)
...
[image: 1653985987327](.\原理篇.assets\1653985987327.png)

[image: 1653986020491](.\原理篇.assets\1653986020491.png)
...
[image: 1653986055253](.\原理篇.assets\1653986055253.png)
...
[image: 1653986172002](.\原理篇.assets\1653986172002.png)
...
[image: 1653986282879](.\原理篇.assets\1653986282879.png)

[image: 1653986217182](.\原理篇.assets\1653986217182.png)
...
[image: 1653986328124](.\原理篇.assets\1653986328124.png)
...
[image: 1653986474927](.\原理篇.assets\1653986474927.png)
...
[image: 1653986642777](.\原理篇.assets\1653986642777.png)

以下是QuickList的和QuickListNode的结构源码：

[image: 1653986667228](.\原理篇.assets\1653986667228.png)

我们接下来用一段流程图来描述当前的这个结构

[image: 1653986718554](.\原理篇.assets\1653986718554.png)
...
[image: 1653986771309](.\原理篇.assets\1653986771309.png)

SkipList（跳表）首先是链表，但与传统链表相比有几点差异：
元素按照升序排列存储
节点可能包含多个指针，指针跨度不同。

[image: 1653986813240](.\原理篇.assets\1653986813240.png)

SkipList（跳表）首先是链表，但与传统链表相比有几点差异：
元素按照升序排列存储
节点可能包含多个指针，指针跨度不同。

[image: 1653986877620](.\原理篇.assets\1653986877620.png)
...
[image: 1653986956618](.\原理篇.assets\1653986956618.png)
...
[image: 1653987103450](.\原理篇.assets\1653987103450.png)

如果存储的字符串是整数值，并且大小在LONG_MAX范围内，则会采用INT编码：直接将数据保存在RedisObject的ptr指针位置（刚好8字节），不再需要SDS了。

[image: 1653987159575](.\原理篇.assets\1653987159575.png)

[image: 1653987172764](.\原理篇.assets\1653987172764.png)

[image: 1653987202522](.\原理篇.assets\1653987202522.png)
...
[image: 1653987240622](.\原理篇.assets\1653987240622.png)
...
[image: 1653987313461](.\原理篇.assets\1653987313461.png)
...
[image: 1653987342550](.\原理篇.assets\1653987342550.png)
...
[image: 1653987388177](.\原理篇.assets\1653987388177.png)

结构如下：

​	[image: 1653987454403](.\原理篇.assets\1653987454403.png)
...
[image: 1653992091967](.\原理篇.assets\1653992091967.png)
...
[image: 1653992121692](.\原理篇.assets\1653992121692.png)

[image: 1653992172526](.\原理篇.assets\1653992172526.png)
...
[image: 1653992238097](.\原理篇.assets\1653992238097.png)

[image: 1653992299740](.\原理篇.assets\1653992299740.png)
...
[image: 1653992339937](.\原理篇.assets\1653992339937.png)

zset集合如下：

[image: 1653992360355](.\原理篇.assets\1653992360355.png)
...
[image: 1653992413406](.\原理篇.assets\1653992413406.png)
...
[image: 1653844970346](.\原理篇.assets\1653844970346.png)

用户的应用，比如redis，mysql等其实是没有办法去执行访问我们操作系统的硬件的，所以我们可以通过发行版的这个壳子去访问内核，再通过内核去访问计算机硬件

[image: 1653845147190](.\原理篇.assets\1653845147190.png)

计算机硬件包括，如cpu，内存，网卡等等，内核（通过寻址空间）可以操作硬件的，但是内核需要不同设备的驱动，有了这些驱动之后，内核就可以去对计算机硬件去进行 内存管理，文件系统的管理，进程的管理等等

[image: 1653896065386](.\原理篇.assets\1653896065386.png)
...
[image: 1653896377259](.\原理篇.assets\1653896377259.png)
...
[image: 1653896687354](.\原理篇.assets\1653896687354.png)
...
[image: 1653897115346](.\原理篇.assets\1653897115346.png)
...
[image: 1653897270074](.\原理篇.assets\1653897270074.png)
...
[image: 1653897490116](.\原理篇.assets\1653897490116.png)
...
[image: 1653898691736](.\原理篇.assets\1653898691736.png)
...
[image: 1653900022580](.\原理篇.assets\1653900022580.png)
...
[image: 1653900721427](.\原理篇.assets\1653900721427.png)
...
[image: 1653902845082](.\原理篇.assets\1653902845082.png)
...
[image: 1653911776583](.\原理篇.assets\1653911776583.png)
...
[image: 1653911877542](.\原理篇.assets\1653911877542.png)

#### 3.0.2 对比

最后用一幅图，来说明他们之间的区别

[image: 1653912219712](.\原理篇.assets\1653912219712.png)
...
[image: 1653982278727](.\原理篇.assets\1653982278727.png)
...
[image: 1653982993020](.\原理篇.assets\1653982993020.png)
...
[image: 1653983341150](.\原理篇.assets\1653983341150.png)
...
[image: 1653983366243](.\原理篇.assets\1653983366243.png)

可以发现，当key的TTL到期以后，再次访问name返回的是nil，说明这个key已经不存在了，对应的内存也得到释放。从而起到内存回收的目的。

Redis本身是一个典型的key-value内存存储数据库，因此所有的key、value都保存在之前学习过的Dict结构中。不过在其database结构体中，有两个Dict：一个用来记录key-value；另一个用来记录key-TTL。

[image: 1653983423128](.\原理篇.assets\1653983423128.png)



[image: 1653983606531](.\原理篇.assets\1653983606531.png)
...
[image: 1653983652865](.\原理篇.assets\1653983652865.png)
...
[image: 1653983978671](.\原理篇.assets\1653983978671.png)
...
[image: 1653984029506](.\原理篇.assets\1653984029506.png)
...
[image: 1653984085095](.\原理篇.assets\1653984085095.png)
...
## 来源 3: Fuwari / `redis/principle/RedisPersistence.md`

- 原始 URL: <https://github.com/DavidHLP/Fuwari/blob/07cee2baf9cee227807dcd68004c5f2493e5ac52/src/content/posts/redis/principle/RedisPersistence.md>
- 本地路径: `redis/principle/RedisPersistence.md`
