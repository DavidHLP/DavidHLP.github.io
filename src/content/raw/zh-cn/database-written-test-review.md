...
capturedAt: 2026-09-13 00:00:00+08:00
sourceType: "user-provided-local-markdown"
sourceUrl: "https://github.com/DavidHLP/DavidHLP.github.io"
immutable: true
tags: [数据库, SQL, Oracle, MySQL, ETL, 笔试复盘]
description: "用户提供的数据库笔试复盘 Markdown 原稿，包含 SQL 语义、连接、游标、字段约束、索引、日期方言、四表练习、Kettle 与超大表处理思路。"
---

# 数据库笔试复盘：从 SQL 语义到批量数据处理

复盘一套数据库笔试题时，真正值得留下的，不只是每道题的答案，而是答案成立的条件：连接保留哪些行，`NULL` 如何参与计算，同一段表达式属于哪种 SQL 方言，以及数据量变大后如何控制成本、验证结果和处理失败。

这篇文章按知识点重组题目，并补充参考 SQL 与容易忽略的边界。Oracle 专属语法单独说明；四表练习与大表操作以 **MySQL 8.4 / InnoDB** 为背景；日期表达式则分别讨论 Spark SQL、Oracle 和 MySQL。
...
因此，在“表、序列、索引、视图”这类选择题中，按上述常见语义，应选**索引**。要区分“数据库可能使用索引执行查询”和“查询把索引当作业务数据源”，两者不是一回事。
...
我的检查习惯是：看到外连接，先在纸上构造一条“完全匹配不到”的记录，再追踪它是否会被后面的过滤条件删除。
...
输出是 **`0`**，不是 `1`。对于显式游标，`%ROWCOUNT` 表示已经提取的记录数，而不是查询结果集的总行数。刚执行 `OPEN`、尚未执行第一次 `FETCH` 时，该值为 `0`；成功提取记录后才会增加。[^11]

这道题提醒我不要仅凭“原表有多少条记录”推断运行结果，而要沿着 `OPEN → FETCH → CLOSE` 的实际执行过程分析状态。
...
对这类题，更完整的回答是：**从高频 SQL 出发，提出候选索引，通过执行计划和实际耗时验证，同时衡量读收益与写成本。**
...
这类题的检查顺序应该是：**确认引擎 → 检查函数和格式符 → 分析返回类型 → 验证跨年边界。**
...
## 九、结语：从“记住答案”转向“证明答案成立”

这次整理后，我更愿意把数据库题目分成三个层次：**语义是否正确、方言是否明确、执行是否可控**。

基础题要能解释为什么不是另一个选项；SQL 题要补上 `NULL`、空结果、重复记录和边界日期；工程题则要继续回答失败后如何恢复、并发时是否一致，以及结果如何对账。

下一轮练习，我会给每道 SQL 都配上至少一个容易出错的反例。能在正常样例之外说明答案为什么仍然成立，比只记住一段查询更有价值。
