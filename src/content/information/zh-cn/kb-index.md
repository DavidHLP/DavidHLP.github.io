...
**6 域速览**：[A 知识库与 AI 工程](#a-知识库与-ai-工程) · [B Java 与并发](#b-java-与并发) · [C 存储与缓存](#c-存储与缓存) · [D 分布式与微服务](#d-分布式与微服务) · [E 基础设施与运维](#e-基础设施与运维) · [F 工程与架构](#f-工程与架构) · [毕业观察](#毕业观察-7-个-provisional) · [已归档](#g-已归档与已废弃deprecated) · [原始来源](#原始来源raw)

### 学习路径（按意图进入）

1. **路径 1 — 新手建立心智**：`llm-wiki-pattern` → `kb-session-ingest-contract` → `java-concurrency-deepdive` / `redis-business-patterns` → `java-online-performance-debug` → `multi-service-readiness`。产出：能独立完成一次小主题 ingest。
...
| 页面 | 毕业需补的 3 个可验证检查 |
|---|---|
| [Headroom 单端口路由综合](/note/headroom-single-port-evolution) | 固定 Headroom 版本下 `/v1/messages` 与 `/v1/responses` 显式路由的最小 contract test；`model_cache` 重建路径；`BindPaths` 回滚验证 |
| [Headroom 路由持久化综合](/note/omp-headroom-persistence) | Named Profile 隔离的独立复现；`model_cache` 作为派生状态的删除重建实验；旧 reconciler 仅在迁移场景的触发条件 |
| [Headroom 与 cc-switch 共存](/note/headroom-cc-switch-coexistence) | `879...→15721` 链路的脱敏抓包/日志证据；单一整理责任的负例（双写冲突）；`BindPaths` 安全边界说明的定版 |
| [OMP Headroom Bridge](/note/omp-headroom-provider-proxy) | loopback proxy、OMP provider route、Codex 共享 provider 的三者共存 smoke test；atomic restore 与 mode 判定；rollback 顺序的可重复步骤 |
| [UltiCode 架构](/note/ulticode) | 判题事务的最小 E2E 实验（port/projection 回滚）；owner 划分的负例；序列化/并发边界的明确声明 |
...
- [AtomicBoolean：原子布尔状态与 CAS 边界](/note/java-atomic-boolean) — 可见性、原子性与 CAS 最小状态转换，何时改用更强同步工具。来源：`legacy-java-atomic-boolean`。
- [AutoCloseable：资源所有权与关闭异常语义](/note/java-auto-closeable) — try-with-resources 的所有权、关闭顺序与异常传播。来源：`legacy-java-auto-closeable`。
- [NullValue：缓存 null 的占位对象与序列化边界](/note/java-null-value) — 空对象与序列化恢复如何避免缓存穿透。来源：`legacy-java-null-value`。
- [Java 并发深度：JUC、CAS、原子类、锁与线程机制](/note/java-concurrency-deepdive) — 以 CAS 串联原子类、锁、协作与引用类型，标注 ABA。来源：`ingest-java-concurrency`。
- [Redisson/Jackson 对 LocalDateTime 的默认序列化行为与写读不对称边界](/note/redis-jackson-java-time) — 写入失败 vs 跨 mapper 读取失败。来源：`redis-jackson-java-time-contract`。
- [Java 线上性能排障：从症状到证据的最小决策树](/note/java-online-performance-debug) — CPU/线程/GC 止损→定位→恢复。来源：`legacy-java-online-performance-debug`。
- [JJWT 0.13.0：显式固定签名算法，按 key 类型验签](/note/jjwt-013-security-api) — `signWith(Key)`/`verifyWith` key 类型与 audience builder。来源：`jjwt-013-security-api-contract`, `jjwt-013-security-api-contract-correction`。

## C. 存储与缓存

- [Redis 业务模式：缓存、分布式锁、消息队列、Feed 流与秒杀](/note/redis-business-patterns) — 以一致性、互斥、语义收敛缓存与业务。来源：`ingest-redis-business`。
...
- [Dubbo + Nacos：先核对注册名、group/version，再做真实 smoke test](/note/dubbo-nacos-runtime) — 接口级/应用级注册、metadata、启动检查。来源：`dubbo-nacos-runtime-registration`, `dubbo-nacos-runtime-registration-correction`。
...
- [Docker 基础：镜像分层、容器操作与 Compose 网络](/note/docker-fundamentals) — 分层、数据卷、网络、Compose 适用边界。来源：`ingest-docker-fundamentals`。
- [containerd TLS：证书信任链与临时跳过验证的决策](/note/containerd-tls-troubleshooting) — CA 信任、运行时配置、Kubernetes 凭证与缓存。来源：`legacy-containerd-tls-troubleshooting`。
- [SSH 内网访问方案：连接方向、暴露面与故障恢复](/note/intranet-penetration-ssh-guide) — Tunnel、Tailscale、FRP 比较。来源：`legacy-intranet-penetration-ssh-guide`。
- [Testcontainers 1.20.6：先排查 Docker API 版本，再判断 daemon 不可用](/note/testcontainers-docker-api) — 固定 `1.32` fallback、`api.version`、400 诊断。来源：`testcontainers-docker-api-negotiation`, `testcontainers-docker-api-negotiation-correction`。
- [百万级终端插件生命周期管理：状态、心跳与受控发布](/note/plugin-lifecycle-management) — 状态、合并心跳、幂等、灰度、熔断。来源：`legacy-plugin-lifecycle-management`。
- *运维排障另见 B 域的 [Java 线上性能排障](/note/java-online-performance-debug)（跨域复用）。*
...
- [ResiCache：Spring Cache 的可编排缓存防护责任链](/note/resicache) — `@RedisCacheable` 责任链 handler 顺序、防护默认关闭与信封迁移。来源：`resicache-project-overview`。
- [UltiCode：在线评测平台的模块化架构与领域边界](/note/ulticode) — **provisional** owner 划分、port/projection、判题事务。来源：`ulticode-project-context`。

### 综合与架构模式

- [OMP 配置分层：模型角色、Agent 覆盖与降级链](/note/omp-config-and-rules-guide) — 配置责任边界与优先级。来源：`legacy-omp-config-and-rules-guide`。
- [OMP Hook 扩展概念：决策点提示、硬阻断与状态桥接](/note/omp-hook-extension-guide) — 软提示、硬护栏、状态栏事件。来源：`legacy-omp-hook-extension-guide`。
- [Headroom 0.34 压缩与检索契约：字段、模式与真实端点验证](/note/headroom-compress-retrieve-contract) — `/v1/compress`、`/v1/retrieve`、CCR 与 contract test。来源：`headroom-0-34-compress-retrieve-contract`。
- [Enhanced UISA：异构节点信息同步的 owner、边界与恢复模型](/note/uisa-architecture-design) — **provisional** owner/可靠性/幂等/异构/恢复权衡。来源：`legacy-uisa-architecture-design`。
- [Headroom 单端口路由综合：入口、动态上游与验证边界](/note/headroom-single-port-evolution) — **provisional** 8787 入口、动态上游与历史假设。来源：`legacy-headroom-single-port-evolution`。
- [Headroom 路由持久化综合：Named Profile、model_cache 与恢复](/note/omp-headroom-persistence) — **provisional** 隔离、reconciler、恢复验证。来源：`legacy-omp-headroom-persistence`。
- [Headroom 与 cc-switch / Claude Code 共存：单一整理与安全边界](/note/headroom-cc-switch-coexistence) — **provisional** 8787/15721/`/v1/messages`/`BindPaths`。来源：`headroom-cc-switch-coexistence-runtime`。
- [Java 后端面试复盘：项目真实性、工程机制与生产证据](/note/java-internship-interview-blog-polished) — **provisional** 能力模型，非题库。来源：`legacy-java-internship-interview-blog-polished`。

## G. 已归档与已废弃（Deprecated）

- [Hindsight 完全本地化部署与 OMP / Codex 统一记忆集成实践（已拆分归档）](/note/hindsight-local-deployment-and-agent-integration) — 历史 445 行巨石页面，已按 LLM-Wiki 原子化原则拆分为部署、接入与排障 3 页。
- [前端与综合随笔：Vue、MyBatis、UniApp、缓存注解与环境排障（已溶解归档）](/note/frontend-mybatis-essays) — 历史 10 篇零散随笔收纳袋，已溶解归档，事实保留在不可变 raw 快照 `ingest-frontend-essays` 中。

---

## 原始来源（raw）

> 49 个 raw 中 10 个为 `legacy-aggregate`（Personal/Fuwari 聚合包，只读）。新增不再使用聚合类型。

<details>
<summary>展开全部 49 个 raw 来源（39 单来源 + 10 legacy-aggregate）</summary>
...
| `ingest-docker-fundamentals` | personal-notes-and-fuwari | [Personal-markdown-notes bbb2126](https://github.com/DavidHLP/Personal-markdown-notes/tree/bbb21260029584d41d1c667f88c5c8e2b761aad9) | Docker 7 篇原文聚合：镜像分层、数据卷、网络与 Compose。 | aggregate |
| `ingest-hbase-foundation` | personal-notes-and-fuwari-overlap | [Personal bbb2126](https://github.com/DavidHLP/Personal-markdown-notes/tree/bbb21260029584d41d1c667f88c5c8e2b761aad9) + [Fuwari 07cee2b](https://github.com/DavidHLP/Fuwari/tree/07cee2baf9cee227807dcd68004c5f2493e5ac52) | HBase 18 篇原文聚合：数据模型、架构、表设计、Shell 与 Java API。 | aggregate |
| `ingest-redis-business` | personal-notes-and-fuwari | [Personal bbb2126](https://github.com/DavidHLP/Personal-markdown-notes/tree/bbb21260029584d41d1c667f88c5c8e2b761aad9) + [Fuwari 07cee2b](https://github.com/DavidHLP/Fuwari/tree/07cee2baf9cee227807dcd68004c5f2493e5ac52) | Redis 业务 20 篇聚合：缓存、分布式锁、队列、Feed 流与秒杀。 | aggregate |
| `ingest-redis-persistence` | personal-notes-and-heima-course | [Personal bbb2126](https://github.com/DavidHLP/Personal-markdown-notes/tree/bbb21260029584d41d1c667f88c5c8e2b761aad9) + [Fuwari 07cee2b](https://github.com/DavidHLP/Fuwari/tree/07cee2baf9cee227807dcd68004c5f2493e5ac52) | Redis 持久化与原理 3 篇：RDB/AOF 与底层结构。 | aggregate |
| `ingest-redis-heima` | personal-notes-and-heima-course | [Personal bbb2126](https://github.com/DavidHLP/Personal-markdown-notes/tree/bbb21260029584d41d1c667f88c5c8e2b761aad9) | 黑马 Redis 6 文件（3 完全重复）聚合：快速入门、实战、多级缓存。 | aggregate |
| `ingest-java-concurrency` | personal-notes-and-fuwari | [Fuwari 07cee2b](https://github.com/DavidHLP/Fuwari/tree/07cee2baf9cee227807dcd68004c5f2493e5ac52) | JUC/Java 15 篇聚合：CAS、原子类、锁与线程机制。 | aggregate |
| `ingest-spring-cloud` | personal-notes-and-fuwari | [Fuwari 07cee2b](https://github.com/DavidHLP/Fuwari/tree/07cee2baf9cee227807dcd68004c5f2493e5ac52) | Spring Boot/Cloud 13 篇聚合：自动装配、事务、网关与可观测性。 | aggregate |
| `ingest-spark-ecosystem` | personal-notes-and-fuwari | [Personal bbb2126](https://github.com/DavidHLP/Personal-markdown-notes/tree/bbb21260029584d41d1c667f88c5c8e2b761aad9) + [Fuwari 07cee2b](https://github.com/DavidHLP/Fuwari/tree/07cee2baf9cee227807dcd68004c5f2493e5ac52) | Spark/大数据 7 篇聚合：引擎概述、运行模式与集群脚本。 | aggregate |
| `ingest-mysql-storage` | personal-notes-and-fuwari | [Personal bbb2126](https://github.com/DavidHLP/Personal-markdown-notes/tree/bbb21260029584d41d1c667f88c5c8e2b761aad9) + [Fuwari 07cee2b](https://github.com/DavidHLP/Fuwari/tree/07cee2baf9cee227807dcd68004c5f2493e5ac52) | MySQL 3 篇聚合：存储引擎与死锁检测。 | aggregate |
...
- [知识库摄入收件箱](/jotting/kb-ingest-todo) — 记录下一批个人项目来源和待回答问题；历史文章、Personal-markdown-notes / Fuwari 两仓库 107 篇原文与 ResiCache / UltiCode 项目 README/CONTEXT 已完成摄入。下一步：执行 Hindsight 拆分与前端随笔溶解（见收敛设计 Phase 2）。
