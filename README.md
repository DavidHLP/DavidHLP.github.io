...
DavidHLPL is a multilingual personal blog and evidence-backed AI knowledge base built on Astro. It follows the content workflow of [ThoughtLite](https://github.com/tuyuritio/astro-theme-thought-lite) while keeping the site's own content, translations, and knowledge-base rules in this repository.

Published notes and jottings are rendered as locale-aware pages and feeds. The site is static and does not require a database, API key, or backend service.
...
| `node scripts/editorial-smoke.mjs` | Validate generated routes, landmarks, layouts, and required assets after a build |
...
| `src/pages/[...locale]/` | Locale-aware routes for the home page, content, feeds, and information pages |
...
The knowledge base uses `zh-cn` as its canonical language. English and Japanese pages are maintained as translations when a multilingual output is required. New published entries are picked up by generated routes and feeds on the next build.
...
The validation workflow runs Biome checks, type checks, unit tests, the static build, editorial smoke checks, committed SVG checks, and browser/print smoke checks for the about page at desktop and mobile widths.
...
This repository is based on the [ThoughtLite](https://github.com/tuyuritio/astro-theme-thought-lite) Astro content-theme foundation. See the [upstream README](https://raw.githubusercontent.com/tuyuritio/astro-theme-thought-lite/refs/heads/main/README.md). The project license remains [GPLv3](LICENSE).

[RhineLabUI](https://github.com/LBEILC/RhineLabUI) is retained as an external reference for possible future archive work. See its [upstream README](https://raw.githubusercontent.com/LBEILC/RhineLabUI/refs/heads/main/README.md). This repository does not currently ship RhineLabUI's interface, assets, or license files.
